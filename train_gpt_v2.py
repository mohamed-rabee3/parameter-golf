...
class Hyperparameters:
    ...
    max_wallclock_seconds = float(os.environ.get("MAX_WALLCLOCK_SECONDS", 4800.0))  # 80 min for 1xH200 (vs 10 min for 8xH100)
    qk_gain_init = float(os.environ.get("QK_GAIN_INIT", 1.5))
    vocab_size = int(os.environ.get("VOCAB_SIZE", 1024))
    num_layers = int(os.environ.get("NUM_LAYERS", 6))
    eval_loops = int(os.environ.get("EVAL_LOOPS", 2))
...
class Rotary(nn.Module):
    def __init__(self, dim: int, base: float = 10000.0, train_seq_len: int = 1024, rope_dims: int = 0,
        ...
        # YaRN frequency interpolation for extended context
...
class GPT(nn.Module):
    def __init__(
        ...
        self._temp = 1.0  # temperature scaling, set by calibrate_temperature()
        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        self.bigram = BigramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim) if bigram_vocab_size > 0 else None
        self.smear = SmearGate(model_dim)
        # Depth recurrence: num_layers = physical layers, effective layers = num_layers * loops
        self.num_layers = num_layers
        # U-Net split: effective layers = num_layers * loops_at_eval
        # For skip connections, use effective_at_eval to size skip_weights
        max_effective = num_layers * eval_loops
        self.num_encoder_layers_eff = max_effective // 2
        self.num_decoder_layers_eff = max_effective - self.num_encoder_layers_eff
        self.num_skip_weights = min(self.num_encoder_layers_eff, self.num_decoder_layers_eff)
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))
        # Per-loop residual gate (learn how much to apply each loop's residual)
        self.loop_gates = nn.Parameter(torch.ones(num_layers, eval_loops, dtype=torch.float32))
        ...
        if rope_dims > 0:
            ...
            for block in self.blocks:
                ...
                block.attn.rotary = Rotary(head_dim, base=rope_base, train_seq_len=1024,
                                          rope_dims=rope_dims, rope_type=rope_type,
                                          yarn_max_len=yarn_max_len)
    ...
    def _softcap(self, logits_proj: Tensor) -> Tensor:
        """Apply logit softcap: poly5 (faster) or tanh."""
        ...
        return s * torch.tanh(logits_proj / s)
    def _run_layers(self, x: Tensor, x0: Tensor, input_ids: Tensor, num_loops: int) -> Tensor:
        """Run transformer blocks with depth recurrence.
        num_loops=1 during training (fast), num_loops=2+ during eval (more depth)."""
        n = self.num_layers
        effective = n * num_loops
        num_enc = effective // 2
        num_dec = effective - num_enc
        v0 = None
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # Encoder pass
        for eff_i in range(num_enc):
            phys = eff_i % n
            loop = eff_i // n
            gate = self.loop_gates[phys, min(loop, self.loop_gates.size(1) - 1)].to(dtype=x.dtype)
            ve = self._get_ve(phys, input_ids, ve_cache)
            x_new, raw_v = self.blocks[phys](x, x0,
                self.qo_bank[phys], self.kv_bank[phys],
                self.kv_bank[n + phys], self.qo_bank[n + phys],
                self.mlp_up_bank[phys], self.mlp_down_bank[phys],
                v_embed=ve, v0=v0)
            if num_loops > 1:
                x = x + gate * (x_new - x)  # gated residual for loop stability
            else:
                x = x_new  # standard direct output when not looping
            if v0 is None and raw_v is not None:
                v0 = raw_v
            skips.append(x)
        # Decoder pass
        for eff_i in range(num_dec):
            phys = (num_enc + eff_i) % n
            loop = (num_enc + eff_i) // n
            gate = self.loop_gates[phys, min(loop, self.loop_gates.size(1) - 1)].to(dtype=x.dtype)
            skip_idx = min(eff_i, self.skip_weights.size(0) - 1)
            if skips:
                x = x + self.skip_weights[skip_idx].to(dtype=x.dtype)[None, None, :] * skips.pop()
            ve = self._get_ve(phys, input_ids, ve_cache)
            x_new, _ = self.blocks[phys](x, x0,
                self.qo_bank[phys], self.kv_bank[phys],
                self.kv_bank[n + phys], self.qo_bank[n + phys],
                self.mlp_up_bank[phys], self.mlp_down_bank[phys],
                v_embed=ve, v0=v0)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x = self.smear(x)
        x0 = x
        num_loops = 1 if self.training else self.eval_loops
        x = self._run_layers(x, x0, input_ids, num_loops)
        x = self.final_norm(x)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        logits_proj = logits_proj / self._temp
...
def eval_val_sliding_ttt(
    ...
    log0(f"ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         ...
         f"ttt_mlp_only={args.ttt_mlp_only} ttt_optimizer={args.ttt_optimizer}")

    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)

    # Selective TTT: freeze all, then unfreeze targeted params
    for p in base_model.parameters():
        p.requires_grad_(False)
    if args.ttt_mlp_only:
        # Only adapt MLP banks (largest capacity, most responsive)
        ...
        # Fallback: freeze first N blocks, unfreeze rest
    ...
    # TTT optimizer: Adam (better for sparse/noisy updates) or SGD (fallback)
    if args.ttt_optimizer == "adam":
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    ...
                    avg_ep_loss = ep_loss_sum / max(ep_loss_count, 1)
                    if _ep > 0:
                        if prev_ep_loss - avg_ep_loss < 1e-4:
                            break
                    prev_ep_loss = avg_ep_loss
...
def main() -> None:
    ...
    if base_model.loop_gates.numel() > 0:
    ...
    eval_model = GPT(
        ...
        softcap_type=args.softcap_type, eval_loops=args.eval_loops,
    ...
    # Temperature calibration: quick grid search on a small validation subset
    if args.temp_scaling:
        cal_tokens_end = min(50_000, val_tokens.numel())
        cal_chunk = val_tokens[:cal_tokens_end + 1].to(dtype=torch.int64, device=device)
        best_temp, best_cal_loss = 1.0, float('inf')
        eval_model.eval()
        with torch.inference_mode():
            for temp in [0.85, 0.88, 0.90, 0.92, 0.95, 1.00]:
                eval_model._temp = temp
                cal_x = cal_chunk[:-1].unsqueeze(0)[:, :args.train_seq_len]
                cal_y = cal_chunk[1:].unsqueeze(0)[:, :args.train_seq_len]
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    logits = eval_model.forward_logits(cal_x)
                cal_loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)).float(),
                                           cal_y.reshape(-1), reduction="mean").item()
                if cal_loss < best_cal_loss:
                    best_temp, best_cal_loss = temp, cal_loss
        eval_model._temp = best_temp
        log0(f"temp_calibration: best_temp={best_temp:.2f} cal_loss={best_cal_loss:.6f}")
